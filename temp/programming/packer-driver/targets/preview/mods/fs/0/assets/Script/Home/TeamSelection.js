    ...
    execute: function () {
      ...
      _export("TeamSelection", TeamSelection = (_dec = ccclass('TeamSelection'), _dec2 = property({
        ...
        onTouchStart(event) {
          for (var i = 0; i < this.dragons.length; i++) {
            if (this.dragons[i].getComponent(UITransform).getBoundingBoxToWorld().contains(v2(event.getUILocation().x, event.getUILocation().y))) {
              ...
              var current = this.node.getComponent(UITransform).convertToNodeSpaceAR(new Vec3(event.getUILocation().x, event.getUILocation().y, 1));
        ...
        onTouchEnd() {
          ...
          for (var i = 0; i < this.DragonPositionNode.length; i++) {
            ...
            if (colide) {
              var pos = this.DragonPositionNode[i].getPosition();
